...
@pytest.mark.config(
    {
        "page_range": [0],
        "use_llm": True,
        "llm_service": "marker.services.claude_agent.ClaudeAgentService",
    }
)
def test_llm_claude_agent(pdf_converter: PdfConverter, temp_doc):
    """Test Claude Agent SDK service initialization.

    Note: This service uses Claude Code CLI authentication,
    so no API key is required in config.
    """
    assert pdf_converter.artifact_dict["llm_service"] is not None
    assert isinstance(pdf_converter.llm_service, ClaudeAgentService)
